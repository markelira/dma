...
const GetPlayerDataSchema = zod_1.z.object({
 ...
 */
exports.getPlayerData = (0, https_1.onCall)({
    cors: true,
    region: 'europe-west1',
}, async (request) => {
    try {
        ...
        v2_1.logger.info('[getPlayerData] Request', { courseId: inputCourseId, lessonId, userId });
        ...
        const [lessonsResult, modulesResult, instructorsResult, progressResult, enrollmentDoc,] = await Promise.all([
        ...
        if (enrollmentDoc.exists) {
            firestore
                ...
                .catch(err => v2_1.logger.warn('[getPlayerData] Failed to update enrollment', err));
        }
        // 9. Build response
        const response = {
            success: true,
        ...
        v2_1.logger.info('[getPlayerData] Success', {
        ...
        v2_1.logger.error('[getPlayerData] Error:', error);
 ...
 */
async function fetchModulesWithLessons(courseId) {
 ...
 */
async function fetchInstructors(instructorIds) {
 ...
 */
async function fetchSourceCourseNames(lessons) {
