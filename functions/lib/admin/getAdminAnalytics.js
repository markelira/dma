...
exports.getAdminAnalytics = (0, https_1.onCall)({
    cors: true,
    region: 'europe-west1',
    memory: '512MiB',
    timeoutSeconds: 60,
    ...
    try {
        ...
        const [usersData, subscriptionsData, teamsData, companiesData, stripeData,] = await Promise.all([
            fetchUserAnalytics(),
        ...
        // Calculate totals
        const totalSubscribers = subscriptionsData.individual.count +
            teamsData.count +
        ...
        v2_1.logger.info('[getAdminAnalytics] Success', {
        ...
        v2_1.logger.error('[getAdminAnalytics] Error:', error);
        return {
            success: false,
...
async function fetchStripeAnalytics() {
    ...
    try {
            ...
            sub.items.data.forEach(item => {
                ...
                if (planInfo) {
                    ...
                    switch (planInfo.name) {
                        case 'monthly':
                            byPlan.monthly += planInfo.monthlyEquivalent;
                            break;
                        case '6-month':
                            byPlan.sixMonth += planInfo.monthlyEquivalent;
                            break;
                        case '12-month':
                            byPlan.yearly += planInfo.monthlyEquivalent;
                            break;
                    }
                }
                else {
        ...
        v2_1.logger.error('[fetchStripeAnalytics] Error:', error);
...
//# sourceMappingURL=getAdminAnalytics.js.map