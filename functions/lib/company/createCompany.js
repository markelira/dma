...
exports.createCompany = v2_1.https.onCall({
    ...
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Must be logged in to create a company');
    }
    ...
    if (!name || name.trim().length < 2) {
        throw new https_1.HttpsError('invalid-argument', 'Company name must be at least 2 characters');
    }
    if (!billingEmail) {
    ...
    if (!emailRegex.test(billingEmail)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid billing email format');
    }
    try {
        ...
        if (!existingCompany.empty) {
            throw new https_1.HttpsError('failed-precondition', 'You already own a company. Contact support to create additional companies.');
        }
        ...
        const adminData = {
            ...
            name: userName,
            role: 'owner',
            permissions: company_1.OWNER_PERMISSIONS,
            status: 'active',
            addedBy: userId,
...
async function sendWelcomeEmail(to, data) {
    ...
    const subject = '🎉 Üdvözlünk az DMA Céges Dashboardon!';
    const htmlContent = `
<!DOCTYPE html>
<html>
<head>
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px; color: #333333; font-size: 16px; line-height: 1.6;">
                A <strong>${data.companyName}</strong> sikeresen létrehozva az DMA platformon.
              </p>
              ...
              <div style="background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 16px 20px; margin: 25px 0; border-radius: 4px;">
                <p style="margin: 0; color: #856404; font-size: 14px; line-height: 1.6;">
                  <strong>💡 Tipp:</strong> Kezdd azzal, hogy meghívod az első alkalmazottaidat, majd rendeld meg számukra a megfelelő képzéseket. Kérdésed van? Írj nekünk bármikor az info@dma.hu címre!
                </p>
              </div>
