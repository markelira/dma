...
exports.sendEmailVerificationCode = (0, https_1.onCall)({
    ...
    try {
        ...
        v2_1.logger.error('Send verification code error:', error);
...
exports.verifyEmailCode = (0, https_1.onCall)({
    ...
    try {
        ...
        if (userDoc.exists) {
            ...
            v2_1.logger.info(`User document updated with emailVerified for user ${userId}`);
        }
        else {
            v2_1.logger.info(`User document not yet created for ${userId}, skipping Firestore update (will be set during registration completion)`);
        }
        // Mark code as used
        await firestore
            .collection('emailVerificationCodes')
        ...
        v2_1.logger.info(`Email verified successfully for user ${userId}`);
        return {
            success: true,
        ...
        v2_1.logger.error('Verify email code error:', error);
        return {
            success: false,
...
exports.resendVerificationCode = (0, https_1.onCall)({
    ...
    try {
        ...
        const result = await exports.sendEmailVerificationCode.run(request);
        return result;
    }
    catch (error) {
        v2_1.logger.error('Resend verification code error:', error);
        return {
            success: false,
