...
export async function handlePaymentSuccess(
  ...
  try {
    ...
    logger.info(`Processing payment success for user: ${userId}, session: ${session.id}`);
    ...
    logger.info(`Payment record created: ${paymentRef.id}`);
    ...
    logger.info(`Payment success handling completed for session: ${session.id}`);
 ...
 */
async function createEnrollment(userId: string, courseId: string): Promise<void> {
  try {
