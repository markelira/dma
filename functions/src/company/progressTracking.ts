...
interface EmployeeProgress {
  ...
  status: 'active' | 'completed' | 'at-risk' | 'not-started' | 'watchlist';
  lastActivityAt?: Date;
  enrolledAt: Date;
  daysActive: number;
  weeksBehind?: number;
...
export const getCompanyDashboard = https.onCall(
  ...
  async (request: CallableRequest<GetCompanyDashboardInput>) => {
    ...
    try {
        ...
        for (let i = 0; i < courseIdArray.length; i += 30) {
          ...
          await Promise.all(coursesSnapshot.docs.map(async (courseDoc) => {
            ...
            if (totalLessons === 0 && courseType === 'ACADEMIA') {
              ...
              const moduleLessonCounts = await Promise.all(
      ...
      const usersData = new Map<string, { displayName: string; email: string; watchlist: string[] }>();
      ...
      const enrolledUserCourses = new Set<string>();
      ...
      for (const companyUserId of companyUserIds) {
        ...
        for (const watchlistCourseId of user.watchlist) {
          ...
          if (enrolledUserCourses.has(enrollmentKey)) continue;
