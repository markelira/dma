...
const MarkCompleteSchema = z.object({
  ...
  analytics: z.object({
    ...
    engagementEvents: z.array(z.any()).optional(),
  }).optional(),
});
 ...
 */
export const markLessonComplete = onCall({
  cors: true,
  region: 'us-central1',
}, async (request) => {
  try {
    ...
    logger.info('Course progress check', {
      userId,
      courseId,
      completedLessons,
      totalLessons,
    ...
    if (courseCompleted) {
      ...
      logger.info('Course completed - enrollment updated', {
        userId,
        courseId
      });
