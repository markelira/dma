...
const StartSessionSchema = z.object({
  ...
  activityType: z.enum(['video', 'reading', 'quiz', 'interactive', 'mixed']),
  deviceType: z.enum(['desktop', 'mobile', 'tablet']).optional(),
});

const EndSessionSchema = z.object({
  sessionId: z.string(),
  progressMade: z.number().min(0).max(100),
});
...
const TrackProgressSchema = z.object({
  ...
  activityType: z.enum(['video', 'reading', 'quiz', 'interactive', 'mixed']).optional(),
});
 ...
 */
async function updateDashboardAnalyticsOnSession(
  userId: string,
  minutesSpent: number,
  activityType: string
): Promise<boolean> {
 ...
 */
async function updateStreakOnSession(userId: string, minutesSpent: number): Promise<boolean> {
  try {
 ...
 */
function checkStreakMilestones(currentStreak: number, existingMilestones: any[]): any[] {
  ...
  for (const threshold of milestoneThresholds) {
    if (currentStreak >= threshold) {
      const exists = milestones.find((m: any) => m.days === threshold);
      if (!exists) {
        milestones.push({
          days: threshold,
          earnedAt: now,
