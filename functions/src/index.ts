  ...
  try {
    ...
    if (initializeSendGrid()) {
      try {
        ...
        logger.error('SendGrid error, falling back to SMTP:', {
...
export const getCoursesCallable = onCall({
  ...
  try {
    ...
    logger.info('[getCoursesCallable] Called', { forImport, includeLessons });
    ...
    for (const doc of snapshot.docs) {
      const data = doc.data();
      if (data.instructorId) instructorIds.add(data.instructorId);
    ...
    if (categoryIds.size > 0) {
      const categoryDocs = await Promise.all(
        Array.from(categoryIds).map(id =>
          firestore.collection('categories').doc(id).get()
        )
      );
    ...
    logger.info(`[getCoursesCallable] Returning ${courses.length} courses`);
