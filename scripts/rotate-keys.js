...
/**
 * DMA Firebase Key Rotation Script (CommonJS version)
 * 
...
class FirebaseKeyRotator {
  constructor() {
    ...
    console.log('🔐 DMA Firebase Key Rotation Tool');
  ...
  readCurrentConfig() {
    if (!fs.existsSync(this.envLocalPath)) {
      console.log('⚠️  No existing .env.local found');
      return null;
    }
  ...
  validateConfig(config) {
    ...
    for (const field of requiredFields) {
      if (!config[field] || config[field].includes('your-') || config[field].includes('new-')) {
        console.log(`❌ Invalid or placeholder value for ${field}: ${config[field]}`);
        return false;
      }
    }
    ...
    if (!config.FIREBASE_API_KEY.startsWith('AIza')) {
      console.log('❌ Firebase API key should start with "AIza"');
      return false;
    }
    ...
    if (!config.FIREBASE_APP_ID.includes(':')) {
      console.log('❌ App ID should contain a colon (:)');
      return false;
    }
  ...
  generateEnvContent(config) {
    const envLines = [
      '# DMA Frontend Environment Variables',
    ...
    envLines.push(
      ...
      'GOOGLE_APPLICATION_CREDENTIALS="../secure/dma-67ab7-firebase-adminsdk.json"',
      '',
      '# Security Settings',
  ...
  async collectNewConfiguration() {
    console.log('\n📋 Please provide new Firebase configuration values:');
  ...
  writeConfiguration(config) {
    ...
    console.log(`✅ Updated ${this.envLocalPath}`);
    ...
    if (fs.existsSync(this.functionsEnvPath)) {
      ...
      console.log(`✅ Updated ${this.functionsEnvPath}`);
    }
  }
  ...
  runSecurityVerification() {
    try {
      ...
      console.log('⚠️  Security verification script not found or failed');
  ...
  async rotateKeys() {
    try {
      console.log('🔄 Starting Firebase key rotation process...\n');
      ...
      if (this.currentConfig) {
        ...
        console.log('📝 No current configuration found, creating new setup');
      }
      ...
      if (!this.validateConfig(newConfig)) {
        console.log('❌ Configuration validation failed. Please check your values.');
        rl.close();
        return;
      }
      ...
      console.log('\n🎉 Firebase key rotation completed successfully!');
      console.log('\n📋 Next steps:');
