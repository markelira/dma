...
export default function PlayerPage() {
  ...
  const flatLessons = useMemo(() => {
    const lessons = modules.flatMap((m: any) =>
      (m.lessons as any[])
        .filter((l: any) => l.status?.toUpperCase() === 'PUBLISHED')
    ...
    console.log('Player: flatLessons calculated:', lessons.length, 'lessons');
    return lessons;
  }, [modules]);
  ...
  // Event handlers (must be before conditional returns to satisfy Rules of Hooks)
  ...
  const handleSelectLesson = useCallback((newLessonId: string) => {
  ...
  if (playerError || lessonError) {
    return (
      <div className="fixed inset-0 bg-background flex items-center justify-center">
        <div className="text-center text-foreground max-w-md px-6">
    ...
    return (
      <div className="fixed inset-0 bg-background flex items-center justify-center">
        <div className="text-center text-foreground max-w-md px-6">
          ...
          <p className="text-muted-foreground mb-4">A kért lecke vagy kurzus nem érhető el.</p>
          <Button onClick={() => router.push('/courses')}>
            Vissza a kurzusokhoz
          </Button>
        </div>
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden font-sans" style={{ colorScheme: 'light' }}>
      {/* Desktop Sidebar */}
            ...
            <div className="space-y-6">
              ...
              <div className="prose prose-slate max-w-none">
                ...
                <p className="text-muted-foreground leading-relaxed">
                  {lesson.description || 'Ebben a leckében gyakorlati példákon keresztül fogod megismerni a témakör részleteit. Kövesd figyelemmel a videót és készíts jegyzeteket!'}
                </p>
                ...
                <ul className="list-disc pl-5 space-y-2 text-muted-foreground">
                  {(lesson.learningOutcomes && lesson.learningOutcomes.length > 0) ? (
                    ...
                    <>
                      <li>Alapvető fogalmak és definíciók tisztázása</li>
                      <li>Gyakorlati példák részletes elemzése</li>
                      <li>Szakmai tippek és bevált gyakorlatok</li>
                      <li>Valós problémák megoldási módszerei</li>
                    </>
                  )}
                </ul>
              </div>
