...
export default function MasterclassPage() {
  ...
  // Filter courses by MASTERCLASS type and user filters
  const filteredCourses = useMemo(() => {
    if (!courses) return [];

    // CRITICAL: Only MASTERCLASS courses
    let result = courses.filter(course => course.courseType === 'MASTERCLASS');
  ...
  // Check if there are any MASTERCLASS courses at all (before user filters)
  const allMasterclassCourses = useMemo(() => {
    if (!courses) return [];
    return courses.filter(course => course.courseType === 'MASTERCLASS');
  }, [courses]);
  ...
  // No MASTERCLASS courses at all
  if (allMasterclassCourses.length === 0) {
    return (
      <AuthProvider>
        <FramerNavbarWrapper />
        <div className="min-h-screen bg-gray-50">
          <div className="space-y-6 px-4 py-16">
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
              <p className="text-gray-500">Nincs megjeleníthető masterclass tartalom</p>
            </div>
          </div>
        </div>
        <Footer border={true} />
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="space-y-8 px-4 sm:px-6 lg:px-8 pt-24 pb-8">
          ...
          {categoryRows.length === 0 && filteredCourses.length > 0 && (
            <CourseCarouselRow
              title="Összes masterclass tartalom"
